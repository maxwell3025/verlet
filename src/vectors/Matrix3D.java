...
public class Matrix3D {
	...
	public static Matrix3D rotx(double radians) {
		return new Matrix3D(new Point3D(1, 0, 0), new Point3D(0, Math.cos(radians), -Math.sin(radians)),
				new Point3D(0, Math.sin(radians), Math.cos(radians)));
	}

	public static Matrix3D roty(double radians) {
		return new Matrix3D(new Point3D(Math.cos(radians), 0, Math.sin(radians)), new Point3D(0, 1, 0),
				new Point3D(-Math.sin(radians), 0, Math.cos(radians)));
	}

	public static Matrix3D rotz(double radians) {
		return new Matrix3D(new Point3D(Math.cos(radians), -Math.sin(radians), 0),
				new Point3D(Math.sin(radians), Math.cos(radians), 0), new Point3D(0, 0, 1));
	}

	public static Matrix3D arbrot(Point3D axis) {
		Matrix3D transformation = rotz(-Math.atan2(axis.y, axis.x));
		transformation = rotx(-Math.atan2(new Point2D(axis.x, axis.y).dist(), axis.z)).transform(transformation);
